...
default. The whole approach is: re-bind the token _primitives_ under one scope
...
> the theme toggle finally _do_ something — the honest path of the two options in
...
- Activation is a single attribute on `<body>` — which `ThemeSetter` already
  ...
  attribute and nothing happens _because no light values exist_. This package
  supplies them.
...
4. In Settings, re-enable the theme selector to offer **Dark** and **Light**
   (the remediation doc had you _remove_ it; now you can keep Dark/Light and drop
...
- **Not a searing white.** Base frame is `oklch(0.952 …)` cool paper, _never_
  `#fff`. Pure-ish white (`ink-3`, `0.994`) is reserved for the cards/editor you
  actually read in.
- **The ramp inverts its _lift_, not its _roles_.** Dark lifts surfaces by
  getting lighter; light lifts them by getting whiter + a soft cool shadow.
  Controls (`ink-4/5`) become gentle toned grays that sit _on_ the white
...
## 4. The one follow-up to be _fully_ complete
...
- Each `--sem-*` ≥ AA as an **icon/border** on neutral surfaces (not as text on
  its own tint — see §3).
- `--tx-on-accent` (near-white) ≥ AA on blue/semantic solid fills;
  `--tx-on-amber` (dark ink) ≥ AA on `--acc-amber` fills.
